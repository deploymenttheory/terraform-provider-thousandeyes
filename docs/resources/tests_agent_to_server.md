...
- `interval` (Number) Interval between test runs in seconds. <!-- probed:7.0.98-t1785744716524 --> The API enforces this field's presence, which the specification does not declare. <!-- /probed -->
...
- `server` (String) The target name or IP address. If a port is set for the test, it is appended to the target name or IP address using a colon. <!-- probed:7.0.98-t1785744716524 --> The API enforces this field's presence, which the specification does not declare. <!-- /probed -->
