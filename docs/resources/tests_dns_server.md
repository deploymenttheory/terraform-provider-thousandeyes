...
- `domain` (String) The target record for the test, with the record type suffixed. If no record type is specified, the test defaults to an ANY record. <!-- probed:7.0.98-t1785744994227 --> The API enforces this field's presence, which the specification does not declare. <!-- /probed -->
...
- `interval` (Number) Interval between test runs in seconds. <!-- probed:7.0.98-t1785744994227 --> The API enforces this field's presence, which the specification does not declare. <!-- /probed -->
...
- `use_public_bgp` (Boolean) Indicate if all available public BGP monitors should be used, when ommited defaults to `bgpMeasurements` value. <!-- probed:7.0.98-t1785744994227 --> Observed: the API assigns true when this is omitted. <!-- /probed -->
