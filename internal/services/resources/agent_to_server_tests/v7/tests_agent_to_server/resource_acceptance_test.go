...
import (
	...
	"github.com/deploymenttheory/terraform-provider-thousandeyes/internal/services/resources/agent_to_server_tests/v7/tests_agent_to_server"
)

// The resource under test, and the helper that asks the API whether it is really there.
const address = "thousandeyes_tests_agent_to_server.test"

var testResource = tests_agent_to_server.TestsAgentToServerTestResource{}

// TestAccResourceTestsAgentToServer_01_Lifecycle exercises the whole lifecycle against a live API.
//
...
func TestAccResourceTestsAgentToServer_01_Lifecycle(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: destroy.CheckDestroyed(testResource, "thousandeyes_tests_agent_to_server", 20*time.Second),
		Steps: []resource.TestStep{
			// Create, from the smallest configuration the schema permits.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					check.That(address).Key("continuous_mode").HasValue("false"),
			...
			// is the most common way a generated provider is subtly wrong.
			{
				...
				// port: it is write-only, carried from configuration, which an import does not have
				// server: it is write-only, carried from configuration, which an import does not have
				// agents: it is write-only, carried from configuration, which an import does not have
				ImportStateVerifyIgnore: []string{"port", "server", "agents"},
			},
			// Update in place, to a configuration that sets the optional attributes too.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					check.That(address).Key("ping_payload_size").HasValue("1"),
