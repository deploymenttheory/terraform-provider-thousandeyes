...
import (
	...
	"github.com/deploymenttheory/terraform-provider-thousandeyes/internal/services/resources/dns_server_tests/v7/tests_dns_server"
)

// The resource under test, and the helper that asks the API whether it is really there.
const address = "thousandeyes_tests_dns_server.test"

var testResource = tests_dns_server.TestsDNSServerTestResource{}

// TestAccResourceTestsDNSServer_01_Lifecycle exercises the whole lifecycle against a live API.
//
...
func TestAccResourceTestsDNSServer_01_Lifecycle(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: destroy.CheckDestroyed(testResource, "thousandeyes_tests_dns_server", 20*time.Second),
		Steps: []resource.TestStep{
			// Create, from the smallest configuration the schema permits.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					check.That(address).Key("recursive_queries").HasValue("false"),
			...
			// is the most common way a generated provider is subtly wrong.
			{
				...
				// dns_servers: it is write-only, carried from configuration, which an import does not have
				...
				ImportStateVerifyIgnore: []string{"dns_servers", "domain", "agents"},
			},
			// Update in place, to a configuration that sets the optional attributes too.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					check.That(address).Key("dns_transport_protocol").HasValue("udp"),
					check.That(address).Key("enabled").HasValue("true"),
