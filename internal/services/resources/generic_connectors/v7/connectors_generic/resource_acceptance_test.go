...
import (
	...
	"github.com/deploymenttheory/terraform-provider-thousandeyes/internal/services/resources/generic_connectors/v7/connectors_generic"
)

// The resource under test, and the helper that asks the API whether it is really there.
const address = "thousandeyes_connectors_generic.test"

var testResource = connectors_generic.ConnectorsGenericTestResource{}

// TestAccResourceConnectorsGeneric_01_Lifecycle exercises the whole lifecycle against a live API.
//
...
func TestAccResourceConnectorsGeneric_01_Lifecycle(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: destroy.CheckDestroyed(testResource, "thousandeyes_connectors_generic", 20*time.Second),
		Steps: []resource.TestStep{
			// Create, from the smallest configuration the schema permits.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					check.That(address).Key("type").HasValue("generic"),
				),
			},
