...
// TestsAPITestResource answers whether a thousandeyes_tests_api is really in the API.
//
...
type TestsAPITestResource struct{}
...
func (TestsAPITestResource) Exists(
	ctx context.Context,
	...
	return exists.Check(ctx, state, func(
		...
		_, err := client.Tests().Api().ByTestId(state.ID).Get(ctx, nil)
