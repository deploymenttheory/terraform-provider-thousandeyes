...
// OperationsWebhookTestResource answers whether a thousandeyes_operations_webhook is really in the API.
//
...
type OperationsWebhookTestResource struct{}
...
func (OperationsWebhookTestResource) Exists(
	ctx context.Context,
	...
	return exists.Check(ctx, state, func(
		...
		_, err := client.Operations().Webhooks().ById(state.ID).Get(ctx, nil)
