...
// TestsDNSTraceTestResource answers whether a thousandeyes_tests_dns_trace is really in the API.
//
...
type TestsDNSTraceTestResource struct{}
...
func (TestsDNSTraceTestResource) Exists(
	ctx context.Context,
	...
	return exists.Check(ctx, state, func(
		...
		_, err := client.Tests().DnsTrace().ByTestId(state.ID).Get(ctx, nil)
