...
// StreamTestResource answers whether a thousandeyes_stream is really in the API.
//
...
type StreamTestResource struct{}
...
func (StreamTestResource) Exists(
	ctx context.Context,
	...
	return exists.Check(ctx, state, func(
		...
		_, err := client.Streams().ById(state.ID).Get(ctx, nil)
