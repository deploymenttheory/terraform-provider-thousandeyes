...
// Create provisions a new thousandeyes_tests_bgp.
func (r *TestsBGPResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	var plan TestsBGPResourceModel
	...
	created, err := r.client.Tests().Bgp().Post(ctx, body, nil)
	if err != nil {
...
// Read refreshes thousandeyes_tests_bgp state from the API.
func (r *TestsBGPResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	var state TestsBGPResourceModel
...
// readState fetches thousandeyes_tests_bgp and maps it onto state.
//
...
func (r *TestsBGPResource) readState(
	ctx context.Context,
	state *TestsBGPResourceModel,
	op errors.Operation,
) (bool, diag.Diagnostics) {
	var diags diag.Diagnostics

	remote, err := r.client.Tests().Bgp().ByTestId(state.ID.ValueString()).Get(ctx, nil)
	if err != nil {
...
func (r *TestsBGPResource) readAfterWrite(
	ctx context.Context,
	state *TestsBGPResourceModel,
	op errors.Operation,
) diag.Diagnostics {
...
func (r *TestsBGPResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	resp.Diagnostics.AddError(
		"Update is not supported by the API",
		"Every change to thousandeyes_tests_bgp forces replacement, so Terraform should never "+
			"have called Update. This is a bug in the provider; please report it.",
	)
}

// Delete removes a thousandeyes_tests_bgp.
func (r *TestsBGPResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var state TestsBGPResourceModel
	...
	err := r.client.Tests().Bgp().ByTestId(state.ID.ValueString()).Delete(ctx, nil)
	if err != nil {
