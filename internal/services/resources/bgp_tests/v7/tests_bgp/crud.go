...
// Update applies configuration changes to an existing thousandeyes_tests_bgp.
func (r *TestsBGPResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var plan, state TestsBGPResourceModel
	...
	_, err := r.client.Tests().Bgp().ByTestId(state.ID.ValueString()).Put(ctx, body, nil)
	if err != nil {
