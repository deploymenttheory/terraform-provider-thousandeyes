...
import (
	...
	"github.com/deploymenttheory/terraform-provider-thousandeyes/internal/services/resources/bgp_tests/v7/tests_bgp"
)

// The resource under test, and the helper that asks the API whether it is really there.
const address = "thousandeyes_tests_bgp.test"

var testResource = tests_bgp.TestsBGPTestResource{}

// TestAccResourceTestsBGP_01_Lifecycle exercises the whole lifecycle against a live API.
//
...
func TestAccResourceTestsBGP_01_Lifecycle(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: destroy.CheckDestroyed(testResource, "thousandeyes_tests_bgp", 20*time.Second),
		Steps: []resource.TestStep{
			// Create, from the smallest configuration the schema permits.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					check.That(address).Key("include_covered_prefixes").HasValue("false"),
					check.That(address).Key("prefix").Exists(),
