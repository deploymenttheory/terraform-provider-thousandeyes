...
// TestsSIPServerTestResource answers whether a thousandeyes_tests_sip_server is really in the API.
//
...
type TestsSIPServerTestResource struct{}
...
func (TestsSIPServerTestResource) Exists(
	ctx context.Context,
	...
	return exists.Check(ctx, state, func(
		...
		_, err := client.Tests().SipServer().ByTestId(state.ID).Get(ctx, nil)
