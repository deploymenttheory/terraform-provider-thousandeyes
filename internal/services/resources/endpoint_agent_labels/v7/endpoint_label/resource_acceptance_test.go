...
import (
	...
	"github.com/deploymenttheory/terraform-provider-thousandeyes/internal/services/resources/endpoint_agent_labels/v7/endpoint_label"
)

// The resource under test, and the helper that asks the API whether it is really there.
const address = "thousandeyes_endpoint_label.test"

var testResource = endpoint_label.EndpointLabelTestResource{}

// TestAccResourceEndpointLabel_01_Lifecycle exercises the whole lifecycle against a live API.
//
...
func TestAccResourceEndpointLabel_01_Lifecycle(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: destroy.CheckDestroyed(testResource, "thousandeyes_endpoint_label", 20*time.Second),
		Steps: []resource.TestStep{
			// Create, from the smallest configuration the schema permits.
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					check.That(address).Key("color").HasValue("#d9d9d9"),
					check.That(address).Key("match_type").HasValue("and"),
				),
			},
