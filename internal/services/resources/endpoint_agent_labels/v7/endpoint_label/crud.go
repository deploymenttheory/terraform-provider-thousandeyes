...
// Create provisions a new thousandeyes_endpoint_label.
func (r *EndpointLabelResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	var plan EndpointLabelResourceModel
	...
	created, err := r.client.Endpoint().Labels().Post(ctx, body, nil)
	if err != nil {
	...
	// record of.
	plan.ID = convert.PtrEnumToFramework(created.GetId())
...
// Read refreshes thousandeyes_endpoint_label state from the API.
func (r *EndpointLabelResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	var state EndpointLabelResourceModel
...
// readState fetches thousandeyes_endpoint_label and maps it onto state.
//
...
func (r *EndpointLabelResource) readState(
	ctx context.Context,
	state *EndpointLabelResourceModel,
	op errors.Operation,
) (bool, diag.Diagnostics) {
	var diags diag.Diagnostics

	remote, err := r.client.Endpoint().Labels().ById(state.ID.ValueString()).Get(ctx, &abstractions.RequestConfiguration[endpoint.LabelsLabelsItemRequestBuilderGetQueryParameters]{QueryParameters: &endpoint.LabelsLabelsItemRequestBuilderGetQueryParameters{Expand: []models.Endpoint_Agent_Labels_API_ExpandLabelOptions{models.FILTERS_ENDPOINT_AGENT_LABELS_API_EXPANDLABELOPTIONS}}})
	if err != nil {
...
func (r *EndpointLabelResource) readAfterWrite(
	ctx context.Context,
	state *EndpointLabelResourceModel,
	op errors.Operation,
) diag.Diagnostics {
...
// Update applies configuration changes to an existing thousandeyes_endpoint_label.
func (r *EndpointLabelResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var plan, state EndpointLabelResourceModel
	...
	_, err := r.client.Endpoint().Labels().ById(state.ID.ValueString()).Patch(ctx, body, nil)
	if err != nil {
...
// Delete removes a thousandeyes_endpoint_label.
func (r *EndpointLabelResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var state EndpointLabelResourceModel
	...
	err := r.client.Endpoint().Labels().ById(state.ID.ValueString()).Delete(ctx, nil)
	if err != nil {
