...
// ConnectorsConjurTestResource answers whether a thousandeyes_connectors_conjur is really in the API.
//
...
type ConnectorsConjurTestResource struct{}
...
func (ConnectorsConjurTestResource) Exists(
	ctx context.Context,
	...
	return exists.Check(ctx, state, func(
		...
		_, err := client.Connectors().Conjur().ById(state.ID).Get(ctx, nil)
