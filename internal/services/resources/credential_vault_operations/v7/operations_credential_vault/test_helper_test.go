...
// OperationsCredentialVaultTestResource answers whether a thousandeyes_operations_credential_vault is really in the API.
//
...
type OperationsCredentialVaultTestResource struct{}
...
func (OperationsCredentialVaultTestResource) Exists(
	ctx context.Context,
	...
	return exists.Check(ctx, state, func(
		...
		_, err := client.Operations().CredentialVault().ById(state.ID).Get(ctx, nil)
