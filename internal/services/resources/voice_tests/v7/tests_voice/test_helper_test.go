...
// TestsVoiceTestResource answers whether a thousandeyes_tests_voice is really in the API.
//
...
type TestsVoiceTestResource struct{}
...
func (TestsVoiceTestResource) Exists(
	ctx context.Context,
	...
	return exists.Check(ctx, state, func(
		...
		_, err := client.Tests().Voice().ByTestId(state.ID).Get(ctx, nil)
