...
// ConnectorsGenericTestResource answers whether a thousandeyes_connectors_generic is really in the API.
//
...
type ConnectorsGenericTestResource struct{}
...
func (ConnectorsGenericTestResource) Exists(
	ctx context.Context,
	...
	return exists.Check(ctx, state, func(
		...
		_, err := client.Connectors().Generic().ById(state.ID).Get(ctx, nil)
